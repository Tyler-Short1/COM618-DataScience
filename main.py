...
def load_data(file):
    ...
    # Handling missing values by imputing with the most frequent value
    imputer = SimpleImputer(strategy='most_frequent')
    df['Data value'] = imputer.fit_transform(df[['Data value']]).ravel()
    df['Footnotes'] = imputer.fit_transform(df[['Footnotes']]).ravel()

    df.drop_duplicates(inplace=True)
    df = df.drop_duplicates(subset=['Area', 'Indicator'])

    return df


# Function to create and plot a network graph
def plot_network_graph(data, top_n=10):
    G = nx.Graph()
    top_policies = data['Indicator'].value_counts().nlargest(top_n).index
    filtered_data = data[data['Indicator'].isin(top_policies)]
    ...
    plt.title('Network Graph of Countries and HIV-Related Policies')
    plt.legend(loc='best')
    st.pyplot(plt)


# Function to plot temporal trends
def plot_temporal_trends(data, top_n=10):
    top_policies = data['Indicator'].value_counts().nlargest(top_n).index
    filtered_data = data[data['Indicator'].isin(top_policies)]
    temporal_data = filtered_data.groupby(['Time Period', 'Indicator']).size().unstack(fill_value=0)

    plt.figure(figsize=(14, 8))
    sns.lineplot(data=temporal_data, dashes=False, markers=True, linewidth=2.5, marker='o')
    plt.title('Temporal Trends in Adoption of Top HIV-Related Policies (2017-2022)')
...
def display_clusters(clusters, country_policies):
    cluster_dict = defaultdict(list)
    for country, cluster in zip(country_policies['Area'], clusters):
        cluster_dict[cluster].append(country)

    for cluster, countries in cluster_dict.items():
        st.write(f"Cluster {cluster + 1}: {', '.join(countries)}")

    return cluster_dict


# Helper function to perform and plot clustering
def perform_clustering(data):
    ...
    display_clusters(clusters, country_policies)
...
def main():
    ...
    if uploaded_file is not None:
        ...
        # Network Graph
            ...
            plot_network_graph(data, top_n=20)  # Show top 20 policies
        ...
        # Temporal Trends Analysis
            ...
            plot_temporal_trends(data, top_n=20)  # Show top 20 policies
        ...
        # Policy and Time Period Interaction Analysis
            ...
            sns.lineplot(x=policy_activity_over_time.index, y=policy_activity_over_time.values, marker='o',
                         linestyle='-',
                         color='b')
